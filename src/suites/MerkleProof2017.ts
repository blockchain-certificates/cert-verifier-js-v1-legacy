...
export default class MerkleProof2017 extends Suite {
  ...
  constructor (props: SuiteAPI) {
    ...
    this.receipt = this.documentToVerify.receipt;
  ...
  private validateProofType (): void {
    ...
    const type = Array.isArray(this.proof.type) ? this.proof.type[0] : this.proof.type;
  ...
  private async computeLocalHash (): Promise<void> {
    this.localHash = await this.executeStep(
      SUB_STEPS.computeLocalHash,
      async () => await inspectors.computeLocalHash(this.documentToVerify.document, Versions.V1_2),
      this.type
    );
  }
  ...
  private async checkAuthenticity (): Promise<void> {
    await this.executeStep(
      SUB_STEPS.checkAuthenticity,
      () => { inspectors.ensureValidIssuingKey(this.issuerPublicKeyList, this.txData.issuingAddress, this.txData.time as string); },
      this.type
    );
  }
}
